function Run-All {
	...
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Changing current user password***" -ForegroundColor Magenta; Change-Current-User-Password } else { Write-Host "Skipping..." -ForegroundColor Red }

	Write-Host "Getting Competition Users" -ForegroundColor Magenta
	if (-not (Test-Path .\users.txt)) { GetCompetitionUsers }

	$confirmation = Prompt-Yes-No -Message "Upgrade SMB? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Upgrading SMB...***" -ForegroundColor Magenta; Upgrade-SMB } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Do Group Management? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Doing Group Management...***" -ForegroundColor Magenta; Group-Management } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Change DA passwords? (y/n)"
	if ($confirmation.toLower() -eq "y") { Change-DA-Passwords; Write-Host "All DA passwords changed" -ForegroundColor Red } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Change non-DA passwords? (y/n)"
	if ($confirmation.toLower() -eq "y") { Change-User-Passwords; Write-Host "All non-DA passwords changed" -ForegroundColor Red } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Disable every user but your own? (y/n)"
	if ($confirmation.toLower() -eq "y") { Mass-Disable; Write-Host "All users disabled but your own" -ForegroundColor Red } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Enter the 'Add Competition Users' function? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Adding Competition Users...***" -ForegroundColor Magenta; Add-Competition-Users } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Enter the 'Remove users from RDP group' function? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Removing users from RDP group...***" -ForegroundColor Magenta; Remove-RDP-Users } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Enter the 'Configure Firewall' function? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Configuring firewall...***" -ForegroundColor Magenta; Configure-Firewall } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Enter the 'Disable unnecessary services' function? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Disabling unnecessary services...***" -ForegroundColor Magenta; Disable-Unnecessary-Services } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Create OUs? (y/n)"
	if ($confirmation.toLower() -eq "y") { try { Write-Host "***Creating OUs***" -ForegroundColor Magenta; Create-OUs; Update-Log "Create OUs" "Executed successfully" } catch { Write-Host $_.Exception.Message -ForegroundColor Yellow; Update-Log "Create OUs" "Failed: $($_.Exception.Message)" } } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Enter the 'Import GPOs' function? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Import GPOs***" -ForegroundColor Magenta; Import-GPOs } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Enable Advanced Auditing and Firewall Logging? (y/n)"
	if ($confirmation.toLower() -eq "y") { Enable-Auditing } else { Write-Host "Skipping..." -ForegroundColor Red }

	Write-Host "It is advisable to configure other desired GPO attributes while the next steps are running."
	Write-Host @"
...
"@
	...
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Configuring Splunk...***" -ForegroundColor Magenta; $SplunkIP = Read-Host "`nInput IP address of Splunk Server"; $SplunkVersion = Read-Host "`nInput OS Version (7, 8, 10, 11, 2012, 2016, 2019, 2022): "; Download-Install-Setup-Splunk -Version $SplunkVersion -IP $SplunkIP } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Enter the 'Configure Sysmon and Connect to Splunk' function? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Configuring Sysmon and Connecting to Splunk...***" -ForegroundColor Magenta; Configure-Sysmon-Connect-Splunk } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Enter the 'Harden IIS' function? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Hardening IIS...***" -ForegroundColor Magenta; Harden-IIS } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Enable UAC with Password? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Enabling UAC...***" -ForegroundColor Magenta; Enable-UAC } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Install EternalBlue Patch? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Installing EternalBlue Patch...***" -ForegroundColor Magenta; Install-EternalBluePatch } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Patch DCSync? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Patching DCSync...***" -ForegroundColor Magenta; Patch-DCSync-Vuln } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Patch Mimikatz? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Patching Mimikatz...***" -ForegroundColor Magenta; Patch-Mimikatz } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Identify and Fix ASREP Roastable Accounts? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Identifying and Fixing ASREP Roastable Accounts...***" -ForegroundColor Magenta; Identify-and-Fix-ASREP-Roastable-Accounts } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Identify and Fix Kerberoastable Accounts? (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Identifying and Fixing Kerberoastable Accounts...***" -ForegroundColor Magenta; Identify-and-Fix-Kerberoastable-Accounts } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Enter the 'Run Windows Updates' function? THIS WILL TAKE A WHILE... (y/n)"
	if ($confirmation.toLower() -eq "y") { Write-Host "`n***Running Windows Updater...***" -ForegroundColor Magenta; Run-Windows-Updates } else { Write-Host "Skipping..." -ForegroundColor Red }

	$confirmation = Prompt-Yes-No -Message "Set Execution Policy back to Restricted? (y/n)"
	if ($confirmation.toLower() -eq "y") { try { Write-Host "`n***Setting Execution Policy back to Restricted...***" -ForegroundColor Magenta; Set-ExecutionPolicy Restricted; Update-Log "Set Execution Policy" "Executed successfully" } catch { Write-Host $_.Exception.Message -ForegroundColor Yellow; Update-Log "Set Execution Policy" "Failed: $($_.Exception.Message)" } } else { Write-Host "Skipping..." -ForegroundColor Red }
...
# Self-contained defaults to avoid external file requirements
if (-not $PortsObject) {
	$PortsObject = @{ ports = @{
		'53'   = @{ description = 'DNS' }
...
# Optional data imports and management
function Import-GPOZip {
	param([Parameter(Mandatory=$true)][string]$Path)
	try {
		if (-not (Test-Path $Path)) { Write-Host "File not found: $Path" -ForegroundColor Yellow; return }
		Copy-Item -Path $Path -Destination ./gpos.zip -Force
		Log-Info "Imported gpos.zip from $Path"
		Write-Host "gpos.zip copied to current directory. Use 'Import GPOs' to apply." -ForegroundColor Green
	} catch {
...
function Import-PatchURLs {
	param([Parameter(Mandatory=$true)][string]$Path)
	try {
		$script:PatchURLsFromJSON_Global = Get-Content -Raw -Path $Path | ConvertFrom-Json
		$script:PatchURLsSource = $Path
		Log-Info "Loaded patchURLs.json from $Path"
		Write-Host "Loaded patch URL mappings." -ForegroundColor Green
	} catch {
		Write-Host $_.Exception.Message -ForegroundColor Yellow
		Write-Host "Error Occurred..."
	}
...
function Import-PortsJSON {
	param([Parameter(Mandatory=$true)][string]$Path)
	try {
		$script:PortsObject = Get-Content -Raw -Path $Path | ConvertFrom-Json
		$script:PortsSource = $Path
		Log-Info "Loaded ports.json from $Path"
		Write-Host "Loaded port definitions." -ForegroundColor Green
	} catch {
		Write-Host $_.Exception.Message -ForegroundColor Yellow
		Write-Host "Error Occurred..."
	}
...
function Reset-Ports-To-Defaults {
	...
	$script:PortsSource = 'defaults'
	Log-Info "Reset ports to defaults"
	Write-Host "Reverted to default ports table." -ForegroundColor Green
}

function Show-Data-Imports-Menu {
	while ($true) {
		Clear-Host
		Write-Host "`n==== Data Imports ====" -ForegroundColor Green
		Write-Host "Current Sources:" -ForegroundColor Cyan
		Write-Host ("  Ports: {0}" -f ($script:PortsSource ? $script:PortsSource : 'defaults'))
		Write-Host ("  Patch URLs: {0}" -f ($script:PatchURLsSource ? $script:PatchURLsSource : 'not loaded'))
		Write-Host ("  GPO Zip present: {0}" -f ((Test-Path ./gpos.zip) ? 'yes' : 'no'))
		Write-Host "`nSelect an option (B to go back):" -ForegroundColor Cyan
		Write-Host "  1) Import gpos.zip from path"
		Write-Host "  2) Import patchurls.json from path"
		Write-Host "  3) Import ports.json from path"
		Write-Host "  4) Reset ports to defaults"
		Write-Host "  5) Show current data summaries"
		$sub = Read-Host "Selection"
		if ($sub -match '^(?i)b$') { break }
		switch ($sub) {
			'1' { $p = Read-Host "Enter full path to gpos.zip"; Import-GPOZip -Path $p }
			'2' { $p = Read-Host "Enter full path to patchurls.json"; Import-PatchURLs -Path $p }
			'3' { $p = Read-Host "Enter full path to ports.json"; Import-PortsJSON -Path $p }
			'4' { Reset-Ports-To-Defaults }
			'5' {
				Write-Host "`nPorts loaded from: $($script:PortsSource ? $script:PortsSource : 'defaults')"
				Write-Host "Patch URLs loaded from: $($script:PatchURLsSource ? $script:PatchURLsSource : 'not loaded')"
				Write-Host "gpos.zip present: $((Test-Path ./gpos.zip) ? 'yes' : 'no')"
			}
			Default { Write-Host "Invalid selection." -ForegroundColor Yellow }
		}
		Write-Host "`nPress Enter to return to Data Imports menu..." -ForegroundColor DarkGray
		Read-Host | Out-Null
	}
}

# Optional: copy local hardening scripts only if they exist locally
try {
	$domainDN = (Get-ADDomain).DistinguishedName
	if (Test-Path "./$localHardeningFile") { cp "./$localHardeningFile" "\\$domainDN\sysvol\$domainDN\$localHardeningFile" }
	if (Test-Path "./$wwHardeningFile") { cp "./$wwHardeningFile" "\\$domainDN\sysvol\$domainDN\$wwHardeningFile" }
} catch {
	Write-Host "Skipping SYSVOL copies (optional): $_" -ForegroundColor Yellow
}
...
# Self-contained function list for logging
$functionNames = @(
	"Initialize Context",
	...
	"Patch DCSync",
	"Patch Mimikatz",
	"Identify & Fix ASREP Roastable Accounts",
	"Identify & Fix Kerberoastable Accounts",
	"Configure Secure GPO",
	"Upgrade SMB",
	"Change Domain Admins Passwords",
	"Change Non-DA Users Passwords",
	"Enable/Disable RDP"
)
...
function Configure-Firewall {
    ...
    try {
        :outer while ($true) {
            Write-Host "Select ports source: (1) Use defaults/current, (2) Load custom ports.json" -ForegroundColor Yellow
            $src = Read-Host "Selection"
            if ($src -eq '2') {
                $p = Read-Host "Enter full path to ports.json"
                if (Test-Path $p) { try { $script:PortsObject = Get-Content -Raw -Path $p | ConvertFrom-Json; $script:PortsSource = $p; Log-Info "Loaded ports.json in Configure-Firewall from $p" } catch { Write-Host "Failed to load ports.json: $_" -ForegroundColor Yellow } } else { Write-Host "File not found; continuing with defaults/current." -ForegroundColor Yellow }
            }
...
function Import-GPOs {
    Write-Host "Use gpos.zip source: (1) Current directory, (2) Choose custom path" -ForegroundColor Yellow
    $choice = Read-Host "Selection"
    $sourceZip = "./gpos.zip"
    if ($choice -eq '2') {
        $p = Read-Host "Enter full path to gpos.zip"
        if (Test-Path $p) {
            $sourceZip = $p
            Log-Info "Using custom gpos.zip: $p"
        } else {
            Write-Host "File not found; falling back to ./gpos.zip" -ForegroundColor Yellow
        }
    }

    Expand-Archive -Path $sourceZip
...
function Install-EternalBluePatch {
    try {
        # Choose mapping source via submenu
        Write-Host "Select patch URL source: (1) Use loaded/local/default, (2) Load custom patchurls.json" -ForegroundColor Yellow
        $src = Read-Host "Selection"
        if ($src -eq '2') {
            $p = Read-Host "Enter full path to patchurls.json"
            if (Test-Path $p) {
                try { $script:PatchURLsFromJSON_Global = Get-Content -Raw -Path $p | ConvertFrom-Json; $script:PatchURLsSource = $p; Log-Info "Loaded patchurls.json in Install-EternalBluePatch from $p" } catch { Write-Host "Failed to load patchurls.json: $_" -ForegroundColor Yellow }
            } else {
                Write-Host "File not found; continuing with loaded/local/default." -ForegroundColor Yellow
            }
        }

        # Prefer imported mapping, fallback to local file, then built-in defaults
        if ($script:PatchURLsFromJSON_Global) { $patchURLsFromJSON = $script:PatchURLsFromJSON_Global }
        elseif (Test-Path $patchURLFile) { $patchURLsFromJSON = Get-Content -Raw -Path $patchURLFile | ConvertFrom-Json }
        else {
            $patchURLsFromJSON = @{ 'Vista'=''; 'Windows 7'=''; 'Windows 8'=''; '2008 R2'=''; '2008'=''; '2012 R2'=''; '2012'='' }
            Write-Host "No patchurls.json provided; URLs may be empty. Consider selecting custom." -ForegroundColor Yellow
        }
        ...
        $patchURL = switch -Regex ($OSVersion) {
            ...
            default { throw "Unsupported or unmapped OS version: $OSVersion" }
        }
		Write-Host $patchURL
...
function Group-Management {
    try {
        Write-Host "`nGroup Management (self-contained)" -ForegroundColor Cyan
        Write-Host "Existing AD Groups:" -ForegroundColor Green
        try {
            Get-ADGroup -Filter * | Select-Object -ExpandProperty Name | Sort-Object | ForEach-Object { Write-Host "  - $_" -ForegroundColor Cyan }
        } catch { Write-Host "Unable to enumerate AD groups: $_" -ForegroundColor Yellow }
        $action = Read-Host "Choose action: (1) Add users to group, (2) Remove users from group, (3) Create group"
        switch ($action) {
            '1' {
                $group = Read-Host "Target group name"
                $users = Get-Comma-Separated-List -category "users" -message "Enter user SamAccountNames (comma-separated, no spaces)"
                foreach ($u in $users) { try { Add-ADGroupMember -Identity $group -Members $u -ErrorAction Stop } catch { Write-Host "Failed for $u: $_" -ForegroundColor Yellow } }
                Write-Host "Users added where possible." -ForegroundColor Green
            }
            '2' {
                $group = Read-Host "Target group name"
                $users = Get-Comma-Separated-List -category "users" -message "Enter user SamAccountNames (comma-separated, no spaces)"
                foreach ($u in $users) { try { Remove-ADGroupMember -Identity $group -Members $u -Confirm:$false -ErrorAction Stop } catch { Write-Host "Failed for $u: $_" -ForegroundColor Yellow } }
                Write-Host "Users removed where possible." -ForegroundColor Green
            }
            '3' {
                $group = Read-Host "New group name"
                New-ADGroup -Name $group -GroupScope Global -ErrorAction SilentlyContinue | Out-Null
                Write-Host "Group ensured: $group" -ForegroundColor Green
            }
            Default { Write-Host "Invalid selection." -ForegroundColor Yellow }
        }
...
function Enable-Auditing {
    try {
        ...
        # Minimal, self-contained audit policy enabling
        $auditCategories = @(
            "Logon/Logoff","Account Logon","Account Management","Policy Change","Privilege Use","Object Access","DS Access","System"
        )
        foreach ($cat in $auditCategories) {
            try { auditpol.exe /set /category:$cat /success:enable /failure:enable | Out-Null } catch { }
        }
...
function Initialize-Context {
	try {
		Write-Host "Initializing context (users, OS, ports, AD computers, GPO)..." -ForegroundColor Cyan
		# Ensure competition users list exists (prompt if missing)
		...
		[string[]]$script:UserArray = Get-Content -Path ".\$usersFile"
		$script:PortsObject = Get-Content -Path ".\$portsFile" -Raw | ConvertFrom-Json
		$script:ADcomputers = Get-ADComputer -Filter * | Select-Object -ExpandProperty Name
		$script:GPOName = "Good-GPO"
...
function Show-Main-Menu {
	Clear-Host
	Write-Host "`n==== Active Directory Hardening Menu ====" -ForegroundColor Green
	Write-Host "Prerequisites:"
    Write-Host "  - (A) Initialize Context BEFORE running user/AD/GPO tasks" -ForegroundColor Yellow
    Write-Host "  - Sysmon->Splunk (12) works best AFTER Configure Splunk (11)" -ForegroundColor Yellow
    Write-Host "  - Configure Secure GPO (20) assumes target GPO exists (e.g., Good-GPO)" -ForegroundColor Yellow
    Write-Host "  - Import GPOs (9) expects gpos.zip present in current directory" -ForegroundColor Yellow
	Write-Host "`nSelect an option by number (or Q to quit):" -ForegroundColor Cyan
	Write-Host "  0) Print Execution Summary"
	Write-Host "  A) Initialize Context (users, OS, ports, AD computers, GPO)"
	Write-Host "  1) Run Full Flow (original y/n prompts)"
	Write-Host "  2) Change Current User Password"
	Write-Host "  3) Add Competition Users"
	Write-Host "  4) Remove RDP Users (harden)"
	Write-Host "  5) Configure Firewall"
	Write-Host "  6) Disable Unnecessary Services"
	Write-Host "  7) Group Management"
	Write-Host "  8) Create OUs"
	Write-Host "  9) Import GPOs"
	Write-Host " 10) Enable Advanced Auditing + Firewall Logging"
	Write-Host " 11) Configure Splunk"
	Write-Host " 12) Configure Sysmon and Connect to Splunk"
	Write-Host " 13) Harden IIS (IIS 7.0 and older)"
	Write-Host " 14) Enable UAC (Admin consent prompts)"
	Write-Host " 15) Install EternalBlue Patch"
	Write-Host " 16) Patch DCSync"
	Write-Host " 17) Patch Mimikatz (WDigest)"
	Write-Host " 18) Identify & Fix ASREP Roastable Accounts"
	Write-Host " 19) Identify & Fix Kerberoastable Accounts"
	Write-Host " 20) Configure Secure GPO"
	Write-Host " 21) Upgrade SMB (enable v2/3, disable v1)"
	Write-Host " 22) Change Domain Admins Passwords"
	Write-Host " 23) Change Non-DA Users Passwords"
	Write-Host " 24) Enable/Disable RDP"
}
...
while ($true) {
	...
	try {
		switch ($choice) {
			...
			'1' { Run-All }
			'2' { Write-Host "`n***Changing current user password***" -ForegroundColor Magenta; Change-Current-User-Password }
			...
			'7' { Write-Host "`n***Doing Group Management...***" -ForegroundColor Magenta; Group-Management }
			'8' { Write-Host "`n***Creating OUs...***" -ForegroundColor Magenta; Create-OUs }
			'9' { Write-Host "`n***Importing GPOs...***" -ForegroundColor Magenta; Import-GPOs }
			'10' { Write-Host "`n***Enabling Advanced Auditing and Firewall logging...***" -ForegroundColor Magenta; Enable-Auditing }
			'11' {
			...
			'12' { Write-Host "`n***Configuring Sysmon and connecting to Splunk...***" -ForegroundColor Magenta; Configure-Sysmon-Connect-Splunk }
			'13' { Write-Host "`n***Hardening IIS...***" -ForegroundColor Magenta; Harden-IIS }
			'14' { Write-Host "`n***Enabling UAC...***" -ForegroundColor Magenta; Enable-UAC }
			'15' { Write-Host "`n***Installing EternalBlue Patch...***" -ForegroundColor Magenta; Install-EternalBluePatch }
			'16' { Write-Host "`n***Patching DCSync...***" -ForegroundColor Magenta; Patch-DCSync-Vuln }
			'17' { Write-Host "`n***Patching Mimikatz (WDigest)...***" -ForegroundColor Magenta; Patch-Mimikatz }
			'18' { Write-Host "`n***Identifying and Fixing ASREP Roastable Accounts...***" -ForegroundColor Magenta; Identify-and-Fix-ASREP-Roastable-Accounts }
			'19' { Write-Host "`n***Identifying and Fixing Kerberoastable Accounts...***" -ForegroundColor Magenta; Identify-and-Fix-Kerberoastable-Accounts }
			'20' { Write-Host "`n***Configuring Secure GPO...***" -ForegroundColor Magenta; Configure-Secure-GPO }
			'21' { Write-Host "`n***Upgrading SMB...***" -ForegroundColor Magenta; Upgrade-SMB }
			'22' { Write-Host "`n***Changing Domain Admins passwords...***" -ForegroundColor Magenta; Change-DA-Passwords }
			'23' { Write-Host "`n***Changing Non-DA passwords...***" -ForegroundColor Magenta; Change-User-Passwords }
			'24' { Write-Host "`n***Enable/Disable RDP...***" -ForegroundColor Magenta; Enable-Disable-RDP }
			Default { Write-Host "Invalid selection." -ForegroundColor Yellow }
		}
	} catch {
		Write-Host $_.Exception.Message -ForegroundColor Yellow
