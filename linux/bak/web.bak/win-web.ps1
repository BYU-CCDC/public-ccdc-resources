...
.DESCRIPTION
    This script performs several hardening and enumeration tasks:
      ...
      - (Dev) Exchange Security configuration backup and hardening (requires Exchange Management Shell)
      - (Dev) Process Monitor – alerts you when a new process starts and prompts you to allow or terminate it
      - (Dev) Service Monitor – alerts you when a new service is detected running and offers to stop it
      
    To run the optional monitors, use the switches -RunProcessMonitor and/or -RunServiceMonitor.
    This script must be run as Administrator.
    
.NOTES
    Credit to Malachi Reynolds / Tyler and others for code inspo 
#>
